    ...
    pub fn handle_end_tag(&mut self, tag: Tag<'_>) -> Control {
        loop {
            ...
            match self.mode {
                ...
                mode!(BeforeHtml) => match tag.name {
                ...
                mode!(BeforeHead) => match tag.name {
                    tag!(HEAD, BODY, HTML, BR) => {
                ...
                mode!(InHeadNoscript) => match tag.name {
                    tag!(NOSCRIPT) => {
