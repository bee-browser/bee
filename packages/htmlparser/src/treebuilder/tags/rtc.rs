...
where
    T: DomTreeBuilder,
{
    pub fn handle_start_rtc(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_start_rtc", mode = ?self.mode);
            ...
            match self.mode {
                ...
                mode!(InBody, InCaption, InCell) => {
                    let ctrl = {
                        ...
                        self.push_html_rtc_element(tag);
                        Control::Continue
                    };
                    match ctrl {
                        Control::Reprocess => continue,
    ...
    pub fn handle_end_rtc(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_end_rtc", mode = ?self.mode);
            let _enter = span.enter();
            match self.mode {
                mode!(Initial) => {
                    let ctrl = {
                    ...
                    let ctrl = {
                        ...
                        loop {
                            ...
                            if context.is_html() && context.open_element.has_same_name(tag.name) {
                                self.close_implied_tags_except_for(tag!(Rtc)); // TODO
