    ...
    T: DomTreeBuilder,
{
    pub fn handle_start_td(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_start_td", mode = ?self.mode);
    ...
    pub fn handle_end_td(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_end_td", mode = ?self.mode);
            let _enter = span.enter();
            match self.mode {
                mode!(Initial) => {
                    let ctrl = {
                    ...
                    let ctrl = {
                        ...
                        loop {
                            ...
                            if context.is_html() && self.inner.has_same_name(element, tag.name) {
                                self.close_implied_tags_except_for(tag!(Td)); // TODO
                ...
                mode!(InCell) => {
                    let ctrl = {
                        if !self.context().has_td_element_in_table_scope() {
                            ...
                            self.close_implied_tags();
                            if !self.context().is_html_element(tag!(Td)) {
                                // TODO: Parse error.
                                tracing::debug!("Parse error");
                            }
                            while !self.context().is_html_element(tag!(Td)) {
                                self.pop_element();
                            }
                            self.pop_element(); // pop an html td element
                            self.pop_active_formatting_elements_up_to_marker();
                            self.switch_to(mode!(InRow));
                            Control::Continue
                        }
