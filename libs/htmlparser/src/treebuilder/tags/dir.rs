...
where
    T: DomTreeBuilder,
{
    pub fn handle_start_dir(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_start_dir", mode = ?self.mode);
            ...
            match self.mode {
                ...
                mode!(InBody, InCaption, InCell) => {
                    let ctrl = {
                        ...
                        self.push_html_dir_element(tag);
                        Control::Continue
                    };
                    match ctrl {
                        Control::Reprocess => continue,
    ...
    pub fn handle_end_dir(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_end_dir", mode = ?self.mode);
            let _enter = span.enter();
            match self.mode {
                mode!(Initial) => {
                    let ctrl = {
                ...
                mode!(InBody, InCaption, InCell) => {
                    let ctrl = {
                        if !self.context().has_dir_element_in_scope() {
                            // TODO: Parse error.
                            tracing::debug!("Parse error");
                            // Ignore the token.
                            tracing::debug!("Ignore the token");
                            ...
                            if !self.context().is_html_element(tag!(Dir)) {
                                // TODO: Parse error.
                                tracing::debug!("Parse error");
                            }
                            while !self.context().is_html_element(tag!(Dir)) {
                                self.pop_element();
                            }
                            self.pop_element(); // pop an html dir element
                            Control::Continue
                        }
                    };
                    match ctrl {
