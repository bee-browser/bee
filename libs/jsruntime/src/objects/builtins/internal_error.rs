...
impl<X> Runtime<X> {
    pub(super) fn create_internal_error_constructor(&mut self) -> ObjectHandle {
        logger::debug!(event = "create_internal_error_constructor");
        debug_assert!(self.internal_error_prototype.is_some());
        let constructor =
            self.create_builtin_function(constructor::<X>, self.internal_error_prototype);
        if let Some(mut prototype) = self.internal_error_prototype {
    ...
    pub(super) fn create_internal_error_prototype(&mut self) -> ObjectHandle {
        logger::debug!(event = "creater_internal_error_prototype");
        ...
        let _ = prototype.define_own_property(
            ...
            Property::data_xxx(Value::String(const_string!(
                jsparser::symbol::builtin::names::INTERNAL_ERROR
            ))),
        );
    ...
    pub(crate) fn create_internal_error(
        &mut self,
        new: bool,
        message: &Value,
        options: &Value,
    ) -> Result<ObjectHandle, Value> {
        logger::debug!(event = "create_internal_error", new, ?message, ?options);
        // TODO(feat): NewTarget
        let mut object = self.create_object(self.internal_error_prototype);
...
extern "C" fn constructor<X>(
    ...
    match runtime.create_internal_error(new, message, options) {
        Ok(value) => {
