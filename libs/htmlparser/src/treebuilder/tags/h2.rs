...
where
    T: DomTreeBuilder,
{
    pub fn handle_start_h2(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_start_h2", mode = ?self.mode);
    ...
    pub fn handle_end_h2(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_end_h2", mode = ?self.mode);
            let _enter = span.enter();
            match self.mode {
                mode!(Initial) => {
                    let ctrl = {
                ...
                mode!(InBody, InCaption, InCell) => {
                    let ctrl = {
                        if !self.context().has_heading_element_in_scope() {
                            ...
                            if !self.context().is_html_element(tag!(H2)) {
                                // TODO: Parse error.
                                tracing::debug!("Parse error");
                            }
