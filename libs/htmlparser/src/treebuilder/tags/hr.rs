...
where
    T: DomTreeBuilder,
{
    pub fn handle_start_hr(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_start_hr", mode = ?self.mode);
    ...
    pub fn handle_end_hr(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_end_hr", mode = ?self.mode);
            let _enter = span.enter();
            match self.mode {
                mode!(Initial) => {
                    let ctrl = {
                    ...
                    let ctrl = {
                        ...
                        loop {
                            ...
                            if context.is_html() && self.inner.has_same_name(element, tag.name) {
                                self.close_implied_tags_except_for(tag!(Hr)); // TODO
