...
where
    T: DomTreeBuilder,
{
    pub fn handle_start_th(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_start_th", mode = ?self.mode);
    ...
    pub fn handle_end_th(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_end_th", mode = ?self.mode);
            let _enter = span.enter();
            match self.mode {
                mode!(Initial) => {
                    let ctrl = {
                    ...
                    let ctrl = {
                        ...
                        loop {
                            ...
                            if context.is_html() && context.open_element.has_same_name(tag.name) {
                                self.close_implied_tags_except_for(tag!(Th)); // TODO
                ...
                mode!(InCell) => {
                    let ctrl = {
                        if !self.context().has_th_element_in_table_scope() {
                            // TODO: Parse error.
                            tracing::debug!("Parse error");
                            // Ignore the token.
                            tracing::debug!("Ignore the token");
                            ...
                            self.close_implied_tags();
                            if !self.context().is_html_element(tag!(Th)) {
                                // TODO: Parse error.
                                tracing::debug!("Parse error");
                            }
                            while !self.context().is_html_element(tag!(Th)) {
                                self.pop_element();
                            }
                            self.pop_element(); // pop an html th element
                            self.active_formatting_element_list
                                .clear_up_to_last_marker();
                            self.switch_to(mode!(InRow));
                            Control::Continue
