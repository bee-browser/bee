...
impl Drop for Validator {
    fn drop(&mut self) {
        assert_eq!(
            self.expected_values.len(),
            self.actual_values.len(),
            "script={} optimize={}",
            self.script,
            self.optimize
        );
...
macro_rules! eval {
    ...
    ($script:expr, $opt:expr, $src:expr; $($expected:expr),+) => {
        {
            let mut runtime = Runtime::with_extension(Validator {
                ...
                optimize: $opt,
                expected_values: vec![$(Value::from($expected)),+],
                actual_values: vec![],
            });
            runtime.enable_scope_cleanup_checker();
    ...
    ($src:expr; $($expected:expr),+) => {
        {
            let src = $src;
            assert_matches!(eval!($src, true, &src; $($expected),+), Ok(_));
            assert_matches!(eval!($src, false, &src; $($expected),+), Ok(_));
        }
    };
    (file: $filename:expr; $($expected:expr),+) => {
        {
            let src = include_str!($filename);
            assert_matches!(eval!($filename, true, src; $($expected),+), Ok(_));
            assert_matches!(eval!($filename, false, src; $($expected),+), Ok(_));
        }
    };
    ($script:expr, $opt:expr, $src:expr; throws: $exception:expr) => {
        assert_matches!(eval!($script, $opt, $src), Err(v) => {
            // Some cases including `f64::NAN` fail in `assert_eq!()`.
            let actual = format!("{v:?}");
            let expected = format!("{:?}", Value::from($exception));
