...
where
    T: DomTreeBuilder,
{
    pub fn handle_start_noembed(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_start_noembed", mode = ?self.mode);
    ...
    pub fn handle_end_noembed(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_end_noembed", mode = ?self.mode);
            let _enter = span.enter();
            match self.mode {
                mode!(Initial) => {
                    let ctrl = {
                    ...
                    let ctrl = {
                        ...
                        loop {
                            ...
                            if context.is_html() && context.open_element.has_same_name(tag.name) {
                                self.close_implied_tags_except_for(tag!(Noembed)); // TODO
