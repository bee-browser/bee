    ...
    T: DomTreeBuilder,
{
    pub fn handle_start_pre(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_start_pre", mode = ?self.mode);
    ...
    pub fn handle_end_pre(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_end_pre", mode = ?self.mode);
            let _enter = span.enter();
            match self.mode {
                mode!(Initial) => {
                    let ctrl = {
                ...
                mode!(InBody, InCaption, InCell) => {
                    let ctrl = {
                        if !self.context().has_pre_element_in_scope() {
                            ...
                            if !self.context().is_html_element(tag!(Pre)) {
                                // TODO: Parse error.
                                tracing::debug!("Parse error");
                            }
                            while !self.context().is_html_element(tag!(Pre)) {
                                self.pop_element();
                            }
                            self.pop_element(); // pop an html pre element
                            Control::Continue
                        }
                    };
                    match ctrl {
