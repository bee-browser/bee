...
where
    T: DomTreeBuilder,
{
    pub fn handle_start_article(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_start_article", mode = ?self.mode);
            ...
            match self.mode {
                ...
                mode!(InBody, InCaption, InCell) => {
                    let ctrl = {
                        ...
                        self.push_html_article_element(tag);
                        Control::Continue
                    };
                    match ctrl {
                        Control::Reprocess => continue,
    ...
    pub fn handle_end_article(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_end_article", mode = ?self.mode);
            let _enter = span.enter();
            match self.mode {
                mode!(Initial) => {
                    let ctrl = {
                ...
                mode!(InBody, InCaption, InCell) => {
                    let ctrl = {
                        if !self.context().has_article_element_in_scope() {
                            // TODO: Parse error.
                            tracing::debug!("Parse error");
                            // Ignore the token.
                        } else {
                            self.close_implied_tags();
                            if !self.context().is_html_element(tag!(Article)) {
                                // TODO: Parse error.
                                tracing::debug!("Parse error");
                            }
                            while !self.context().is_html_element(tag!(Article)) {
                                self.pop_element();
                            }
                            self.pop_element(); // pop an html article element
                        }
                        Control::Continue
                    };
                    match ctrl {
