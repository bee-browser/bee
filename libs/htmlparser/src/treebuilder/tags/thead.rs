...
where
    T: DomTreeBuilder,
{
    pub fn handle_start_thead(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_start_thead", mode = ?self.mode);
            ...
            match self.mode {
                ...
                mode!(InTable) => {
                    let ctrl = {
                        ...
                        self.push_html_thead_element(tag);
    ...
    pub fn handle_end_thead(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_end_thead", mode = ?self.mode);
            let _enter = span.enter();
            match self.mode {
                mode!(Initial) => {
                    let ctrl = {
                    ...
                    let ctrl = {
                        ...
                        loop {
                            ...
                            if context.is_html() && context.open_element.has_same_name(tag.name) {
                                self.close_implied_tags_except_for(tag!(Thead)); // TODO
                ...
                mode!(InTableBody) => {
                    let ctrl = {
                        if !self.context().has_thead_element_in_table_scope() {
                            // TODO: Parse error.
                            tracing::debug!("Parse error");
                            // Ignore the token.
                        } else {
