...
where
    T: DomTreeBuilder,
{
    pub fn handle_start_menu(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_start_menu", mode = ?self.mode);
            ...
            match self.mode {
                ...
                mode!(InBody, InCaption, InCell) => {
                    let ctrl = {
                        ...
                        self.push_html_menu_element(tag);
                        Control::Continue
                    };
                    match ctrl {
                        Control::Reprocess => continue,
    ...
    pub fn handle_end_menu(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_end_menu", mode = ?self.mode);
            let _enter = span.enter();
            match self.mode {
                mode!(Initial) => {
                    let ctrl = {
                ...
                mode!(InBody, InCaption, InCell) => {
                    let ctrl = {
                        if !self.context().has_menu_element_in_scope() {
                            // TODO: Parse error.
                            tracing::debug!("Parse error");
                            // Ignore the token.
                            tracing::debug!("Ignore the token");
                            ...
                            if !self.context().is_html_element(tag!(Menu)) {
                                // TODO: Parse error.
                                tracing::debug!("Parse error");
                            }
                            while !self.context().is_html_element(tag!(Menu)) {
                                self.pop_element();
                            }
                            self.pop_element(); // pop an html menu element
                            Control::Continue
                        }
                    };
                    match ctrl {
