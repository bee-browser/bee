...
import { f, formatBeeNumber, formatBeePoint, h, t } from './helper.js';
...
export default class LayoutPropView extends Widget {
  constructor() {
    ...
    this.tabBar_.appendTab({
      id: 'basic',
      label: 'Basic',
      content: this.basicView_,
    });
    this.tabBar_.appendTab({
      id: 'styles',
      label: 'Styles',
      content: this.stylesView_,
    });
    this.tabBar_.appendTab({
      id: 'box-model',
      label: 'Box Model',
      content: this.boxModelView_,
    });
    this.tabBar_.appendTab({
      id: 'layer',
      label: 'Layer',
      content: this.layerView_,
    });
...
class LayoutBasicPropView extends Widget {
  ...
  renderProp_(prop) {
    return h(
      'section',
      { class: 'layout-prop-section' },
      h(
        'table',
        { class: 'layout-prop-table' },
...
class LayoutStylesView extends Widget {
  ...
  renderProp_(style) {
    return h('section', { class: 'layout-prop-section' }, this.renderStyleTable_(style));
  }
...
class LayoutBoxModelView extends Widget {
  ...
  renderProp_(dim) {
    return h('section', { class: 'layout-prop-section' }, this.renderDimensionTable_(dim));
  }
...
class LayoutLayerView extends Widget {
  ...
  renderProp_(layer) {
    return f(
      this.renderLayerBasicSection_(layer),
      h('hr'),
  ...
  renderLayerBasicSection_(layer) {
    return h(
      ...
      h(
        ...
        renderProp_(
          'ID',
          layer.id.toString(16).toUpperCase(),
        ),
        renderProp_(
          'Owner ID',
          layer.ownerId.toString(16).toUpperCase(),
        ),
        renderProp_(
          'Container ID',
          layer.containerId.toString(16).toUpperCase(),
        ),
        renderProp_(
          'New Stacking Context',
          layer.newStackingContext,
        ),
        renderProp_(
          'Stack Level',
          layer.stackLevel,
  ...
  renderLayerPositionSection_(layer) {
    return h(
      ...
      h(
        ...
        renderProp_(
          'Position',
          formatBeePoint(layer.position),
        ),
        renderProp_(
          'Static Position',
          formatBeePoint(layer.staticPosition),
        ),
        renderProp_(
          'Extra Offset',
          formatBeePoint(layer.extraOffset),
        ),
      ),
    );
  }
