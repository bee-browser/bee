...
where
    T: DomTreeBuilder,
{
    pub fn handle_start_footer(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_start_footer", mode = ?self.mode);
            ...
            match self.mode {
                ...
                mode!(InBody, InCaption, InCell) => {
                    let ctrl = {
                        ...
                        self.push_html_footer_element(tag);
                        Control::Continue
                    };
                    match ctrl {
                        Control::Reprocess => continue,
    ...
    pub fn handle_end_footer(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_end_footer", mode = ?self.mode);
            let _enter = span.enter();
            match self.mode {
                mode!(Initial) => {
                    let ctrl = {
                ...
                mode!(InBody, InCaption, InCell) => {
                    let ctrl = {
                        if !self.context().has_footer_element_in_scope() {
                            // TODO: Parse error.
                            tracing::debug!("Parse error");
                            // Ignore the token.
                        } else {
                            self.close_implied_tags();
                            if !self.context().is_html_element(tag!(Footer)) {
                                // TODO: Parse error.
                                tracing::debug!("Parse error");
                            }
                            while !self.context().is_html_element(tag!(Footer)) {
                                self.pop_element();
                            }
                            self.pop_element(); // pop an html footer element
                        }
                        Control::Continue
                    };
                    match ctrl {
