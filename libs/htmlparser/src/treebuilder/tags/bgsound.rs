...
where
    T: DomTreeBuilder,
{
    pub fn handle_start_bgsound(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_start_bgsound", mode = ?self.mode);
            ...
            match self.mode {
                ...
                mode!(
                    ...
                    let ctrl = {
                        self.push_html_bgsound_element(tag);
                        self.pop_element();
                        if tag.self_closing {
    ...
    pub fn handle_end_bgsound(&mut self, tag: &Tag<'_>) -> Control {
        loop {
            let span = tracing::debug_span!("handle_end_bgsound", mode = ?self.mode);
            let _enter = span.enter();
            match self.mode {
                mode!(Initial) => {
                    let ctrl = {
                    ...
                    let ctrl = {
                        ...
                        loop {
                            ...
                            if context.is_html() && self.inner.has_same_name(element, tag.name) {
                                self.close_implied_tags_except_for(tag!(Bgsound)); // TODO
